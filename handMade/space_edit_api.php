...
$sid = intval($_POST['space_sid']);
$sql = " SELECT * FROM `space_list` WHERE `space_sid`=$sid ";
$row = $pdo->query($sql)->fetch();

$space_image_path = $row['space_image_path'];
$space_logo_path = $row['space_logo_path'];


if ( !empty($_FILES['space_image_path']['name']) and count($_FILES['space_image_path']['name'])>0) {
    $image_path = [];
    ...
    foreach ($_FILES['space_image_path']['name'] as $k => $v) {
        ...
        $image_path[] = $new_filename_space . $new_ext_space;
       
    }
    if(!empty($image_path)){
        $space_image_path = json_encode($image_path, JSON_UNESCAPED_UNICODE);
    }
...
if (!empty($_FILES['space_logo_path']['name'])) {
    ...
    $space_logo_path =  $new_filename . $new_ext;
}
